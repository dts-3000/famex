...
export default function CelebCard({ celeb, price, history, buzz, delistWarning, holding, onBuy, onSell }) {
  const [qty, setQty] = useState(1)
  const chg = getPriceChange(history, price)
  const isUp = chg >= 0
  const isDelisted = price <= 0

  // Delist risk indicator
    ...
    >
      ...
      {delistRisk && (
        <div style={{
          ...
          color: delistRisk === 'high' ? 'var(--red)' : '#ff8c00',
          textAlign: 'center',
        }}>
      ...
      <div>
        ...
        {/* Buzz vs base indicator */}
        <div style={{ fontSize: 10, color: 'var(--text3)', fontFamily: 'var(--font-mono)', marginTop: 4 }}>
          Base: {celeb.buzzBase} ·{' '}
      ...
      {!isDelisted && (
        <div style={{ fontSize: 10, color: 'var(--text3)', fontFamily: 'var(--font-mono)', textAlign: 'center' }}>
          {qty > 1 ? `Trading ${qty} shares will move price ~${(Math.sqrt(qty) * 0.15).toFixed(2)}%` : 'Trades move the market price'}
        </div>
      )}
