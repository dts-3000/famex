...
import { CELEBRITIES, SECTORS, UPDATE_INTERVAL, STARTING_CASH, initState, tickMarket, fmt, fmtChange } from './data.js'
...
export default function App() {
  ...
  const handleBuy = useCallback((id, qty) => {
    setState(prev => {
      ...
      const h = prev.holdings[id]
      const newAvg = (h.avgCost * h.qty + total) / (h.qty + qty)
      const c = CELEBRITIES.find(x => x.id === id)
      showToast(`Bought ${qty}x ${c.name} for ${fmt(total)}`, 'buy')
      return {
        ...prev,
        cash: prev.cash - total,
        holdings: { ...prev.holdings, [id]: { qty: h.qty + qty, avgCost: newAvg } }
      }
    })
  }, [showToast])
  ...
  const handleSell = useCallback((id, qty) => {
    setState(prev => {
      ...
      const c = CELEBRITIES.find(x => x.id === id)
      showToast(`Sold ${qty}x ${c.name} for ${fmt(total)}`, 'sell')
      return {
        ...prev,
        cash: prev.cash + total,
        holdings: { ...prev.holdings, [id]: { qty: h.qty - qty, avgCost: h.qty - qty === 0 ? 0 : h.avgCost } }
      }
    })
  }, [showToast])

  const portfolioValue = CELEBRITIES.reduce((s, c) => s + state.prices[c.id] * state.holdings[c.id].qty, 0)
  const totalWorth = state.cash + portfolioValue
  const pnl = totalWorth - STARTING_CASH
  const pnlUp = pnl >= 0

  const visibleCelebs = sector === 'All' ? CELEBRITIES : CELEBRITIES.filter(c => c.sector === sector)
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{
        ...
        <Ticker prices={state.prices} history={state.history} />
      </header>
      ...
      <main style={{ flex: 1, maxWidth: 1200, margin: '0 auto', padding: '20px', width: '100%' }}>
        ...
        {tab === 'portfolio' && (
          <Portfolio holdings={state.holdings} prices={state.prices} />
        )}
